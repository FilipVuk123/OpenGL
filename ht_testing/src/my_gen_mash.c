...
static void my_gen_sphere(my_sphere_t *s){
...
static void my_gen_window(my_window_t *win){
...
void my_sphere_free(my_sphere_t *sph){
    free(sph->Vs); free(sph->Is);
}

void my_window_free(my_window_t *win){
    free(win->Vs); free(win->Is);
}

my_window_t my_create_window(const float radius, const float angleX, const float angleY, float x, float y, float z){
    my_window_t window;
    window.radius = radius; window.angleY = angleY; window.angleX = angleX; window.x = x; window.y = y; window.z = z;
    my_gen_window(&window);
    return window;
}

my_sphere_t my_create_sphere(const float radius, const float sectors, const float stacks){
    my_sphere_t sphere;
    sphere.radius = radius; sphere.sectors = sectors; sphere.stacks = stacks;
    my_gen_sphere(&sphere);
    return sphere;
}
