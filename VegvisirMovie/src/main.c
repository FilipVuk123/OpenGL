...
int main(){
    ...
    if (orqa_init_glfw(3,3)) return -1;
    ...
    orqa_bind_buffer_set_data(GL_ARRAY_BUFFER, VBOs[0], sph.numVertices*sizeof(float), sph.Vs, GL_STATIC_DRAW);
    orqa_bind_buffer_set_data(GL_ELEMENT_ARRAY_BUFFER, EBOs[0], sph.numTriangles*sizeof(int), sph.Is, GL_STATIC_DRAW);
    ...
    orqa_video_reader_open_file(&vr_state, "./data/360videoRGB.mp4");
    
    orqa_bind_texture(textures[0]);
    orqa_generate_texture_from_buffer(GL_TEXTURE_2D, GL_RGB, vr_state.width, vr_state.height, GL_RGB, GL_UNSIGNED_BYTE, NULL);
    ...
    while (!glfwWindowShouldClose(window)){ // render loop
        ...
        orqa_bind_texture(textures[0]);
        orqa_update_texture_from_buffer(GL_TEXTURE_2D, 0, 0, vr_state.width, vr_state.height, GL_RGB, GL_UNSIGNED_BYTE, orqa_video_reader_read_frame(&vr_state));
        orqa_bind_vertex_object_and_draw_it(VAOs[0], GL_TRIANGLES, sph.numTriangles);
...
static void orqa_process_input(GLFWwindow *window){ 
    ...
    if(orqa_get_key(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) // closes window on ESC
